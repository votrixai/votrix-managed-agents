...
| Workspace isolation | Implemented | API keys resolve to `CurrentWorkspace`; core queries and object keys are workspace-scoped. | Organization membership, role inheritance, SSO, and policy evaluation are outside the open core. |
...
| Environment CRUD | Implemented | Stores and validates cloud, self-hosted, and local environment configuration. | Network, package, and resource policies are not enforced unless an operator-provided remote sandbox enforces them. |
...
| Session-local agent changes | Partial | Idle-session updates can replace selected agent configuration without creating a new agent version. | Full create-time and update-time overrides for model, system, tools, MCP servers, and skills are not implemented uniformly in runtime compilation. |
...
| File API | Partial | Upload/list/retrieve/download/delete, workspace deduplication, upload limits, local EICAR checks, and S3-compatible storage exist. | Production malware scanning, retention, generated-file discovery, and remote-sandbox mount lifecycle remain incomplete. |
| Session resources | Partial | File, GitHub repository, and memory-store response unions are persisted; file copies and limits are covered. | Real mount/read-only enforcement is delegated to the sandbox adapter; GitHub checkout is not a complete production integration. |
...
| Memory-store API | Partial | Path memories, limits, optimistic preconditions, immutable versions, deletion history, and filtering are implemented. | Runtime memory is bounded context injection, not a fully mounted Claude-compatible memory filesystem with agent writeback. |
...
| Area | Status | Current VMA behavior | Material difference or remaining work |
| --- | --- | --- | --- |
| Runtime kernel | Implemented | Session turns compile an immutable VMA agent snapshot into a run-scoped Deep Agents graph, stream it, and persist translated output. Real-graph tests cover execution and checkpoint resume. The deterministic `local` backend is explicit test behavior. | Deep Agents is a beta library and is not itself a durable multi-tenant service. Exact Claude behavior remains partial. |
...
| MCP runtime | Partial | Runtime matches vault credentials by normalized MCP URL and can construct remote MCP tools with static authorization headers. Missing credentials emit non-terminal session errors. | OAuth refresh, discovery, revocation, reconnect policy, server capability drift, and complete MCP approval event mapping are missing. |
| Custom skills at runtime | Partial | Versioned archives are selected, bounded, traversal/symlink checked, extracted into `/skills/custom`, and passed to Deep Agents skill directories. | Deep Agents reads Agent Skills `SKILL.md`; it does not guarantee the same system prompt, bundled content, cache behavior, or security policy as Claude. |
...
| Memory at runtime | Partial | A bounded memory snapshot is materialized at each store's mount path with an `AGENTS.md` source, and read-only write denials are applied when the backend has no shell. | Runtime changes are not synchronized back to memory-store versions; execute-capable backends must enforce access themselves; no semantic index exists. |
...
| Safe default backend | Implemented | Without a provider factory, `StateBackend` supplies checkpointed file state and does not expose shell execution. | It is not a cloud sandbox and does not enforce environment network/package policy. |
| Remote sandbox | Gap | `VMA_SANDBOX_FACTORY` defines the integration boundary. | VMA ships no production container/VM provider. Operators must supply isolation and lifecycle management. See [sandbox runtime](./sandbox-runtime.md). |
...
| Object persistence | Partial | S3-compatible storage supports files and skill archives. | Retention, malware quarantine, regional replication, and customer-managed keys are not productized. |
...
| RBAC and SSO | Gap | Not part of the workspace-scoped open core. | Required for a managed enterprise service. |
