...
- Every non-no-op create, update, and delete creates an immutable memory version; versions survive after their memory is deleted and paginate across the complete store history.
...
- Redaction permanently clears content, path, digest, and size fields from both stored representations of the targeted version. Public responses never expose the internal snapshot. The current live head version cannot be redacted.
