...
```text
Cloud Run web/API (maxScale=1, WEB_CONCURRENCY=1)
...
VMA's live-preview bus and per-session execution lock are process-local today. A second process or replica could lose transient preview frames and race execution for the same Session. Do not increase `WEB_CONCURRENCY` or `maxScale` until VMA has a distributed session lock with fencing and a tenant-scoped preview broker. The single-instance shape is an explicit correctness constraint, not an availability guarantee.
...
Cloud Run's writable filesystem is ephemeral and must not hold authoritative session, event, file, Skill, or checkpoint state. Configure S3-compatible object storage for uploaded bytes and Skill archives.
...
Cloud environments run through the current inline MVP path. Scheduled Deployment resources expose an idempotent due-schedule tick, but no production Cloud Scheduler, Cloud Run Job, or always-on scheduler invokes it yet. Creating a scheduled Deployment does not by itself guarantee execution until that integration is added.
